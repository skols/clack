var React       = require("react"),
    ReactDOM    = require("react-dom");
    
var Users = React.createClass({
    render: function() {
        var userList = [];
        
        if (this.props.users) {
            userList = this.props.users.map(function(user,i) {
                return (
                   <li key={i}><img src={"https://twitter.com/" + user + "/profile_image"} className="user_profile-pic" /> {user} <img className="connection_icon" src="data:image/png;base64,"/></li>
                ) 
            });
        }
        
        return (
            <div>
                <h2 id="users_online_header">Channel Members ({userList.length})</h2>
                <ul className="user_list">
                    {userList}
                </ul>
            </div>
        )
    }
});

module.exports = Users;